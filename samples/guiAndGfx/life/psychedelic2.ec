...
class GameOfLife : Window
{
   ...
   Timer tickTimer
   {
      delay = 0 /*0.05 /*1 /*0.01*/, started = false /*true*/, userData = this;
               ...
               */
               ...
               switch(state)
               {
                  ...
                  default:
                     ...
                     if(alive > 1 && alive < 300)
                     {
                         //alive = 0; //nCount != 6; //0; // Death
                         //alive = nCount < 4;
                         // alive = nCount < 5 ? (alive + 5) : 0;

                          alive = (nCount >= 200 || nCount <= 400) ? (alive + 3) : 4;
                         //alive = (nCount >= 380 || nCount <= 400) ? (alive + 3) : 0;
                        ...
                        alive = nCount >= 250 && nCount < 350 ? 3 : alive; // Birth
                        //alive = nCount >= 250 && nCount < 350 ? 2 : alive; // Birth
                        //alive = nCount >= 325 && nCount < 380 ? 2 : alive; // Birth
