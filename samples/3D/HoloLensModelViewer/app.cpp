...
DLLIMPORT int E3DViewer_Init(int argc, const char ** argv, const wchar_t* widePath);
DLLIMPORT void E3DViewer_Cycle(int w, int h);
DLLIMPORT void E3DViewer_Render();
DLLIMPORT void E3DViewer_Terminate();
};

//std::wstring installedPath = Windows::ApplicationModel::Package::Current->InstalledLocation->Path->Data();
...
using namespace HoloLensModelViewer;
...
void App::SetWindow(CoreWindow^ window)
{
    ...
    try
    {
        ...
        if (locator != nullptr)
        {
            ...
            throw Exception::CreateException(E_FAIL, L"No Holographic");
    }
...
void App::Run()
{
    while (!mWindowClosed)
    {
        if (mWindowVisible)
        {
            ...
            E3DViewer_Cycle(panelWidth, panelHeight);
...
void App::InitializeEGLInner(Platform::Object^ windowBasis)
{
    ...
    const EGLint surfaceAttributes[] =
    {
        ...
        //EGL_EXPERIMENTAL_PRESENT_PATH_ANGLE, EGL_TRUE,
        EGL_NONE
    };
   ...
   {
      const char * argv[] = { "modelViewer",  };
      int argc = sizeof(argv) / sizeof(argv[0]);
      std::wstring installedPath = Windows::ApplicationModel::Package::Current->InstalledLocation->Path->Data();
      E3DViewer_Init(argc, argv, installedPath.c_str());
   }
}
