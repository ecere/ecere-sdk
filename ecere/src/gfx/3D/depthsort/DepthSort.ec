...
struct SortPrimitive
{
   ...
   void depend(SortPrimitive b, Pool<TopoNode> nodesPool, Pool<TopoEdge> edgesPool)
   {
...
static void topoSortPrimitives(Pool<TopoNode> nodesPool, Pool<TopoEdge> edgesPool, SortPrimitive * triangles, uint nTriangles)
{
   if(nTriangles)
   {
      OldList input { };
...
struct SortDataPtr
{
   ...
   void free()
   {
      ...
      Pool<TopoEdge> edgesPool = this != null ? data[1] : null;
      Pool<TopoNode> nodesPool = this != null ? data[2] : null;
   ...
   int maxFullSort, bool fullProjCheck)
{
   ...
   if(maxFullSort)
   {
      ...
      Pool<TopoEdge> edgesPool = sortData != null ? sortData.data[1] : null;
      Pool<TopoNode> nodesPool = sortData != null ? sortData.data[2] : null;
